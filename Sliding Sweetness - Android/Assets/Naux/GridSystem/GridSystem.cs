...
namespace Naux.GridSystem
{
    ...
    public class GridSystem<T>
    {
        ...
        internal static GridSystem<T> GenerateGrid(Vector2Int gridSize, float cellSize, GridType gridType, Vector3 origin, bool drawGridLines = false)
        {
            switch (gridType)
            {
                case GridType.XY_Plane:


                    origin = new(origin.x - (gridSize.x * cellSize / 2), origin.y - (gridSize.y * cellSize / 2), origin.z);
                    return new GridSystem<T>(gridSize, cellSize, origin, new XYConverter(), drawGridLines);
                case GridType.XZ_Plane:
                    origin = new(origin.x - (gridSize.x * cellSize / 2), origin.y, origin.z - (gridSize.y * cellSize / 2));
                    return new GridSystem<T>(gridSize, cellSize, origin, new XZConverter(), drawGridLines);
                default:
                    throw new ArgumentException("Invalid GridType provided");
            }
        }

        /// <summary>
        /// Initializes a new instance of the GridSystem class.
        /// </summary>
        /// <param name="gridSize">The size of the grid.</param>
        /// <param name="cellSize">The size of each cell in the grid.</param>
        /// <param name="origin">The origin position in the world.</param>
        /// <param name="converter">The coordinate converter used for grid position calculations.</param>
        /// <param name="drawGridLines">Whether to draw grid lines for visualization.</param>
        ...
        /// <summary>
        /// Draws the grid lines for visualization in the editor.
        /// </summary>
        void DrawGridLines()
        {
            var _duration = 120f;
        ...
        /// <summary>
        /// Retrieves the value stored in the grid at the specified world position.
        /// </summary>
        /// <param name="worldPosition">The world position to retrieve the value from.</param>
        /// <returns>The value at the specified grid cell.</returns>
        internal T GetValue(Vector3 worldPosition)
        {
            Vector2Int _grid = GetXYCoordinates(worldPosition);
            return GetValue(_grid.x, _grid.y);
        }
        ...
        /// <param name="x">The x-coordinate of the grid cell.</param>
        /// <param name="y">The y-coordinate of the grid cell.</param>
        /// <param name="value">The value to set at the specified grid cell.</param>
